const Success = () => {
  return (
      <section className="w-full">
      <div className="h-96 bg-bottom bg-cover bg-[url('https://www.teahub.io/photos/full/48-485738_-contact-us.jpg')]"></div>
      <div className="max-w-5xl mx-auto px-6 sm:px-6 lg:px-8 mb-12 min-h-[33.5rem]">
        <div className="bg-white w-full shadow rounded-md p-8 sm:p-12 -mt-72 h-[12rem] flex justify-center items-center">
          <p className="text-3xl font-bold leading-7 text-center">
            Gracias por contactarse!
          </p>
        </div>
      </div>
    </section>
  )
};

export default Success;
